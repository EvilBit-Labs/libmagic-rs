...
| **Fail-safe defaults** | `#![forbid(unsafe_code)]` enforced project-wide. Buffer access defaults to bounds-checked `.get()` returning `None` rather than panicking. Invalid magic rules are skipped, not executed. |
...
|-----|----------|---------------|--------|
| CWE-787 | Out-of-bounds write | Rust ownership prevents writes to unowned memory. `#![forbid(unsafe_code)]` eliminates raw pointer writes. | Mitigated |
...
| Pinned toolchain | Rust 1.91.0 via `rust-toolchain.toml` |
