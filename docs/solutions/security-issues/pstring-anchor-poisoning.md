...
- **Keep dual-purpose helpers in sync.** When `read_pstring`/`read_string`/`read_string_exact`/`read_string16` change their bounds enforcement or dispatch arms, the corresponding `pstring_bytes_consumed`/`string_bytes_consumed`/`string16_bytes_consumed` partners must change too. Each new pattern arm in `read_typed_value_with_pattern` (e.g., the `Value::Bytes` arm added in PR #233) needs a matching arm in `bytes_consumed_with_pattern_bounded` or the relative-offset anchor mis-advances silently. The rule has now caught four incidents:
  ...
  4. Issue #498 — the read and the consume side each measured a *different quantity* rather than one omitting an arm: walked byte count vs the pattern's declared length, and UTF-8 byte length vs UCS-2 code units (see [`docs/solutions/design-patterns/two-coinciding-quantities-hide-a-conflation-bug.md`](../design-patterns/two-coinciding-quantities-hide-a-conflation-bug.md))

  **One deliberate exception, added by #498.** The "anchor advance must equal what the read consumed from the buffer" framing above holds for every case in this doc, but the flagged-`string` and `search` paths now advance by the magic rule's own *declared* pattern length, which can legitimately exceed the bytes the file contained and can point past `buffer.len()`. That is GNU `file`'s `moffset()` behavior, verified against the real binary, and the out-of-bounds anchor is safe because `resolve_relative_offset` bounds-checks and returns `BufferOverrun` for a graceful non-match. Read and consume still agree with *each other*; what changed is that neither is derived from the buffer. Do not "fix" it back to a buffer-derived count or re-add a clamp — see GOTCHAS S2.6 and S6.8.
...
- `GOTCHAS.md` S3.8 — Relative Offsets: Global Anchor, No Save/Restore (load-bearing context for any future contributor touching the evaluator's anchor). The anchor is not monotonic, despite an earlier title here saying so: a `Relative(-N)` match can move it earlier.
