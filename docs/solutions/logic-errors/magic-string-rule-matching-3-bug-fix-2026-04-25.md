...
- [`docs/solutions/design-patterns/multi-agent-review-surfaces-cross-cutting-consistency-gaps-2026-04-25.md`](../design-patterns/multi-agent-review-surfaces-cross-cutting-consistency-gaps-2026-04-25.md) -- **follow-on.** A multi-agent review of PR #233 surfaced 5 cross-cutting consistency gaps in the same change set, including the `compare_values` trichotomy parallel to this fix's `apply_equal` cross-type policy (now implemented per Gap 1 of that doc), and the missing `bytes_consumed_with_pattern` `Value::Bytes` arm that this fix's read-side change introduced (Gap 2). Read it after this doc when extending any cross-cutting helper.
...
- [`GOTCHAS.md`](../../../GOTCHAS.md) S2.3 (`Value` Exhaustive Matches) -- the cross-type `Value::Bytes` vs `Value::String` equality policy added by Fix 3 is now codified there as a load-bearing convention. The follow-on doc extended the policy to `compare_values` for trichotomy parity.
- [`GOTCHAS.md`](../../../GOTCHAS.md) S6.4 -- now documents the dual read-mode dispatch (`read_string_exact` for pattern comparison, `read_string` for `x` any-value scans) and the "pick consciously" guidance for future contributors adding new code paths.
- [`AGENTS.md`](../../../AGENTS.md) "Currently Implemented" / string section -- updated to describe the dual read-mode dispatch and bareword string escape interpretation.
