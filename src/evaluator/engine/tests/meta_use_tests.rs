...
// ---------------------------------------------------------------------------
// use-vs-indirect result framing (issue #471, GOTCHAS S3.10)
//
...
// pinned here end to end, not just at the helper.
//
...
fn test_use_indirect_result_is_base_relative_jpeg_shape() {
    ...
    let outer = vec![use_rule_indirect_at(
        "inner",
        OffsetSpec::Indirect {
            base_offset: 2,
            base_relative: false,
...
fn test_use_indirect_result_at_zero_base_is_unrebased() {
    ...
    let outer_rule = use_rule_indirect_at(
        "inner",
        OffsetSpec::Indirect {
            base_offset: 2,
            base_relative: false,
