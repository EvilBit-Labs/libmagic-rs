...
/// GOTCHAS S13.2 (refined): a message-less `default` match must not stop
/// evaluation before a later, message-bearing sibling is reached.
#[test]
fn test_default_message_less_match_does_not_stop_at_first_match() {
    let rules = vec![
        default_rule("", vec![]),
        byte_eq_rule(0, 0xAA, "Real message"),
    ];
    let mut context = EvaluationContext::new(EvaluationConfig::default());
    let matches = evaluate_rules(&rules, &[0xAAu8], &mut context).unwrap();

    assert_eq!(
        matches.len(),
        2,
        "both the default and the real rule should match"
    );
