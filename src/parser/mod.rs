...
//! # Format Detection
//!
//! Before parsing, the module can detect the format of magic files:
...
//!
//! Use `detect_format()` to determine the format before parsing.
//!
//! # Example
//!
//! ```ignore
...
/// # Errors
///
/// Returns `ParseError::InvalidSyntax` if the path doesn't exist or cannot be read.
///
/// # Notes
///
...
pub fn detect_format(path: &Path) -> Result<MagicFileFormat, ParseError> {
    ...
    if !path.exists() {
        return Err(ParseError::invalid_syntax(
            0,
            format!("Magic file path does not exist: {}", path.display()),
        ));
    }
    ...
    let mut file = std::fs::File::open(path)
        .map_err(|e| ParseError::invalid_syntax(0, format!("Failed to open magic file: {e}")))?;
