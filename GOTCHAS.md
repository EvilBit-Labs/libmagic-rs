...
**Known gap (not yet fixed):** a string-family bareword whose value *begins* with a backslash (e.g. `\<?xml\ version=`) is routed through `parse_value`'s hex/mixed-ascii branch (S3.12) **before** the `parse_bare_string_value` fallback is ever reached, and that branch's `parse_mixed_hex_ascii` stops at the first literal space -- so `\<?xml\ version=` is still captured as a truncated `Value::Bytes` (`\<?xml\`) with ` version=` leaking. The `parse_bare_string_value` drop-backslash fix above therefore does NOT yet fix XML/plist detection (`0 string \<?xml\ version=`); that needs the escaped-space (`\ `) handling pushed into `parse_mixed_hex_ascii` / the string-family routing in `parse_magic_rule`. Tracked for a follow-up.
