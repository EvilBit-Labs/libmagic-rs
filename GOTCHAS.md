...
**A `use` rule with an Indirect offset needs a *different* result rule, and it is NOT implemented.** jpeg's `>>(2.S+2) use jpeg_segment` reads a segment *length*, so its next segment is `use-site + length + 2` -- the result must be base-relative there, whereas mach-o's `indirect` result must stay absolute. Measured on a real JFIF file: segments sit at 2, 20, 150; from the segment at 20 the length 128 at byte 22 must resolve to 150, i.e. `base(20) + 128 + 2`. The two cases are distinguished only by the rule's meta-type (`use` continues the caller's frame; `indirect` re-enters at a fresh absolute position). rmagic currently applies the absolute rule to both, so jpeg's `jpeg_segment` walk still resolves to the wrong position and the `baseline, precision N, WxH, components N` tail is missing. Tracked under #471; do not "fix" it by making `indirect` results base-relative too -- that breaks Mach-O.
...
The suppression is gated on the indirect rule being message-bearing. With no message there is nothing to continue, so the ordinary separator applies (`macintosh:203`'s bare `>>128 indirect x` keeps its pre-existing behavior). Implemented via `engine::output::attach_no_separator_to_first`, which marks the first match that actually renders text so a leading message-less match cannot swallow the marker. The same helper serves the `use`-site `\b` in S14.4.
