...
**Known intentional divergence:** rmagic can out-resolve GNU `file` here. `/usr/lib/dyld`'s `arch[1].offset` is 1,064,960 -- past libmagic's ~1 MB read buffer -- so `file` prints a bare `[arm64e]` while rmagic mmaps the whole file and prints `[arm64e:Mach-O 64-bit dynamic linker arm64e]`. That is more detail, not a regression; do not "fix" it to match `file`.
