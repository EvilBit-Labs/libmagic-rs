...
**The narrow graceful-skip is now variant-keyed, not string-keyed (issue #391 item 2).** Two dedicated `TypeReadError` variants carry the exactly-two skippable conditions: `MissingPatternOperand { reason }` (a pattern-bearing type -- `Regex`/`Search`/flagged `String` -- evaluated without a usable `String`/`Bytes` operand; `reason` is still one of the three constants `REGEX_MISSING_PATTERN_MSG` / `SEARCH_MISSING_PATTERN_MSG` / `FLAGGED_STRING_MISSING_PATTERN_MSG` for diagnostics) and `RegexCompileError { detail }` (a regex compile failure, including the `REGEX_COMPILE_SIZE_LIMIT` CWE-1333 DoS guard). The three engine catch sites (the top-level dispatch match in `evaluator/engine/mod.rs::evaluate_rules`, `evaluate_children_or_warn`, and the inline child-recursion match under a successful parent) match on `TypeReadError::is_pattern_skip()` -- `matches!(self, MissingPatternOperand { .. } | RegexCompileError { .. })` -- and skip the rule (non-match) instead of aborting the whole file, logged via the shared `log_pattern_operand_skip` helper: `debug!` for the missing-operand case, `warn!` for a regex compile failure (keyed on `TypeReadError::is_regex_compile_failure()`, so a malicious or pathological magic file's rejection stays visible in logs even though evaluation of the rest of the file continues). Any OTHER cause -- an unwired `TypeKind` variant, a non-Equal/NotEqual operator applied to a pattern-bearing type (S2.4), a `Meta` variant read as a value -- is a `TypeReadError::UnsupportedType` and still falls through and propagates fatally at all three sites. The old string predicates (`is_missing_pattern_operand` / `is_regex_compile_failure` matching on the free-form `type_name` string) were retired: the contract is identical but now compiler-enforced by variant identity. Do not extend `is_pattern_skip` to cover `UnsupportedType`; that would silently swallow genuine capability gaps this contract exists to surface.
...
### 2.6 Search Anchor Advance Is Match Position Plus DECLARED Pattern Length, Not the Walked Length (Measured Correction, issue #498)

An earlier revision of this section claimed the anchor advance was `match_idx + matched_len` (the byte-walk's consumed count) and cited `moffset()`'s `vlen = m->vallen`. That citation was right about the source line but wrong about what it means: `m->vallen` is the pattern's own DECLARED length, not however many file bytes the comparator walked to confirm the match. For an UNFLAGGED pattern the two coincide (`memmem` is byte-exact, so `matched_len == pattern.len()`), which is why the old claim looked correct and passed its own tests. They diverge only under `/w` (`compact_optional_whitespace`) or `/W` (`compact_whitespace`), where the file may have zero, one, or many whitespace bytes at a position the pattern spells with one. Measured against real `file`-5.41: `search/32/w` pattern `A\ B` (declared length 3) over buffer `A   Bqz` (a 3-space run) resolves a relative-offset child to index 3 -- match position (0) plus DECLARED length (3) -- not the walked match-end (5). `search_bytes_consumed` now derives the advance from `pattern.len()` (or the `/T`-trimmed length when trim is set), never from `ScanHit.matched_len`. `matched_len` itself is unchanged and still correctly reports the walked count -- it just has no callers computing anchor advance anymore. This is the search-side twin of the `string`-side correction at S6.8; both were fixed together (R6) because they share the same `moffset()` misreading. See `src/evaluator/types/search.rs::test_search_flag_w_anchor_uses_declared_pattern_length_not_walked_length` and `src/evaluator/types/tests/bytes_consumed_pattern.rs::test_bytes_consumed_search_flagged_w_declared_length_not_walked_length` for the oracle-measured regression tests.

An earlier fix in this same section (before the declared-length correction above) established that the anchor advances to the byte just past the matched pattern, not to `range` (the window size): GNU `file`'s `FILE_SEARCH` branch of `moffset()` computes `o = ms->search.offset + vlen - offset` where `ms->search.offset` has already been advanced by `idx` (the match index inside the window) in `magiccheck`. That match-position-plus-length shape is still correct; only the "length" operand changed from walked to declared.

**`/s` start-anchor branch (issue #235).** When `SearchFlags.start_anchor` is set, `search_bytes_consumed` returns `match_idx` instead of the past-end advance — relative-offset children resolve to the byte at match-START rather than match-END. This mirrors libmagic's `REGEX_OFFSET_START` flag (`src/file.h::STRING_SEARCHEND`) which zeros the matched-length contribution in `moffset()`. The flag affects ONLY anchor advance; the match decision itself is unchanged, and it is orthogonal to (unaffected by) the declared-vs-walked-length correction above. Required for TGA-footer detection (`images:114`), sfnt name table parsing (`fonts:260`), and any rule whose children walk backwards from the matched signature. Tests at `src/evaluator/types/search.rs::test_search_with_start_anchor_returns_match_start_index` and `tests/search_flag_conformance_tests.rs::search_s_anchors_relative_child_at_match_start` pin the contract.

**Declared-length policy under `/T`.** When `SearchFlags.trim` is set, the pattern is trimmed of leading/trailing ASCII whitespace at the dispatcher boundary and the comparator sees only the trimmed slice. The anchor advance uses that trimmed pattern's length, not the untrimmed `pattern.len()` — using the untrimmed length would advance the anchor past bytes the comparator never inspected. The flagged-string path (`read_pattern_match` in `evaluator/types/mod.rs`) follows the same trim-at-boundary contract; search.rs inlines its own `trim_ascii_whitespace` because the module-level helper is private.

**No clamp against the remaining buffer.** An earlier revision clamped the advance to `buffer.len() - offset` "to defend against overflow on adversarial input." That clamp is gone: `saturating_add` alone prevents integer overflow, and a `/w`/`/W` declared length can legitimately exceed however many bytes the file actually walked, so the returned advance may legitimately point past `buffer.len()`. A downstream relative-offset child at an out-of-bounds offset simply fails to match (S15.1) rather than silently landing at a clamped-but-wrong position. This mirrors the flagged-`string` anchor at S6.8, which never had a buffer clamp.
...
Similarly, `strength.rs::calculate_default_strength` and `types/mod.rs::bytes_consumed_with_pattern_bounded` treat all `Meta(_)` variants uniformly (zero strength contribution, zero bytes consumed) via catch-alls, so they do not need per-variant arms.
...
**Two paths deliberately advance the anchor past what the buffer actually contained, not from it.** A flag-walked `string` match (S6.8) and a `search` match under `/w`/`/W` (S2.6) both advance by the rule's own DECLARED pattern length rather than by the walked byte count, matching GNU `file`'s `moffset()`. This is the one exception to "re-derives consumption from the buffer" above; see S2.6 and S6.8 for the measured detail.
...
Inside a `MetaType::Use` subroutine body, `OffsetSpec::Absolute(n)` with `n >= 0` resolves to `base_offset + n`, where `base_offset` is the use-site offset. `EvaluationContext::base_offset` tracks this; `evaluate_use_rule` saves and restores it around the subroutine call, alongside `last_match_end`. This matches magic(5) / libmagic semantics: a subroutine written as `>0 search/12 ABC` invoked via `>>64 use part2` scans starting at file position 64, not 0. Negative absolute offsets (`FromEnd`-style) and `Relative(&N)` offsets are unaffected -- they already have well-defined reference frames. An `Indirect` offset's dereferenced *result* is likewise unaffected, but its pointer-read *site* IS biased -- see the next paragraph. Callers of `offset::resolve_offset_with_base` pass `context.base_offset()`; the `base_offset` field is gated behind `pub(crate)` accessors so external consumers cannot inject arbitrary offset bias.

**The base also biases an indirect offset's pointer-read SITE -- never its dereferenced result (issue #378).** `OffsetSpec::Indirect.base_offset` says *where to read the pointer*; the value read through it is an absolute file position. Inside `use mach-o` invoked at 8, `>(8.L)` must read `arch[0].offset` at `base(8) + 8 = 16`, then treat that value (e.g. 16384) as an absolute offset -- not `8 + 16384`. Verified against a real fat header: `arch[i].offset` lives at `use-site + 8` for every `i`, and its value is the absolute mach-header position. Before the fix the site was resolved absolutely, so rmagic read `arch[0].cputype` at 8, got `0x01000007`, and overran the buffer at 16777223 -- the per-arch inner classification never ran. Two exclusions, both load-bearing: `(&N.X)` (`base_relative`) is NOT biased because `SubroutineScope::enter` already seeds the anchor to the use-site, so adding the base again double-counts; and a negative `base_offset` keeps from-end semantics, mirroring the `Absolute` arm. Implemented by `apply_subroutine_base` in `evaluator/offset/indirect.rs`. Regressions: `test_resolve_offset_with_base_biases_indirect_pointer_read_site`, `test_resolve_offset_with_base_does_not_bias_indirect_result`, and `test_resolve_offset_with_base_does_not_double_bias_base_relative_indirect` (`evaluator/offset/mod.rs`).

**A `use` rule's Indirect offset resolves its RESULT in the caller's frame; an `indirect` rule's stays absolute.** jpeg's `>>(2.S+2) use jpeg_segment` reads a segment *length*, so the next segment is `use-site + length + 2`: measured on a real JFIF, segments sit at 2, 20, 150, and from 20 the length 128 at byte 22 must resolve to 150. mach-o's `>(8.L) indirect x` is the opposite -- `arch[i].offset` is an absolute file position (16384), which must not be biased. Same `(N.T+adj)` syntax, opposite result rules; the only discriminator is the rule's meta-type, which matches the engine's shapes (`Indirect` slices the buffer and resets the base via `AnchorScope`; `Use` sets the base and does not slice). Implemented by `apply_use_result_base` in `evaluator/engine/subroutine.rs`, which excludes a non-`Indirect` offset, a zero base, and `&(N.X)` (`result_relative`, whose anchor addition already carries the use-site).
...
### 12.4 A `permissions:` Block Scopes the Ambient `GITHUB_TOKEN`, Not Other Credentials

A workflow's `permissions:` block scopes the ambient `GITHUB_TOKEN`. The one key that is not a `GITHUB_TOKEN` scope is `id-token: write`, which gates retrieval of an OIDC token -- a separate credential -- and confers no resource access itself (four workflows here use it: `release-plz.yml:14`, `release.yml:118`, `docs.yml:13`, `scorecard.yml:31`). A step authenticating with any other credential is outside the block entirely, so "this workflow writes something" is not on its own a reason to grant more than `contents: read`. This repo has hit it twice: `release-plz.yml`'s release job keeps `contents: read` because its writes go through a minted GitHub App token (#449), and `compatibility.yml` keeps `contents: read` despite an `actions/upload-artifact` step, because `upload-artifact` v4+ authenticates with `ACTIONS_RUNTIME_TOKEN` (#456). The upstream mechanism is not verifiable from this tree; the in-repo evidence is `ci.yml`'s `coverage` job, which uploads via the same pinned action SHA under an inherited `contents: read` and passes on every run.

The matching validation trap: `compatibility.yml`'s upload is gated `if: failure()`, so a green PR run never executes it. A run never validates a step whose gate is false on that run -- `if: failure()` here, equally `if: cancelled()` or an unmet event/fork guard. The trap is the gate's value on this run, not the presence of a condition: an `if: always()` step does execute on a green run. Cite a job that already exercises the same mechanism under the same constraint instead, matching on the pinned SHA rather than the action name. Note also that `actionlint` is a local-only pre-commit hook (no CI job runs it), so `just ci-check` is the only lint gate for a workflow edit.

See `docs/solutions/design-patterns/workflow-permissions-govern-github-token-only.md` for the full worked case.
...
### 14.4 `use <name>` Emits the Subroutine's `name`-Line Message (No Separator); `use`'s Own Message Is Dropped

A magic(5) `name` line can carry its own description, and GNU `file` emits that description when the subroutine is invoked via `use` -- ahead of the body's matches and attached with **no separating space**. rmagic previously dropped it at **two** layers, so subroutine name-line fragments silently vanished: Mach-O universal `0 name mach-o \b [` (the leading `[` of `[x86_64:...]`), `0 name matlab4 Matlab v4 mat-file`, `0 name algol_68 Algol 68 source text`, `0 name msdos-driver DOS executable (`, `0 name xbase-prf dBase Printer Form`, etc. (107 of 338 `name` directives in the system DB carry a message.)

The fix spans three layers; all three are load-bearing:

1. **Parse (`parser/grammar/mod.rs::parse_name_or_use_meta`).** The trailing text after the identifier is handled **differently for `name` vs `use`**, matching GNU `file`: `name` PRESERVES it (positioned so the caller's `parse_message` captures it as `rule.message`), while `use` still DROPS it up to end-of-line (a use-site has no message slot -- verified: `0 use foo bar` renders no `bar`). The previous code dropped it for both, so `rule.message` was empty for every `name` line and the later layers had nothing to emit.
2. **Hoist (`parser/name_table.rs::extract_name_table`).** The `NameTable` value is a `Subroutine { name_message: String, rules: Arc<[MagicRule]> }` -- the name-line message is captured alongside the body. `NameTable::get(name)` still returns `Arc<[MagicRule]>` (unchanged contract); `NameTable::name_message(name)` returns the description (`None` for a bare `name <id>` or unknown name).
3. **Emit (`evaluator/engine/subroutine.rs::evaluate_use_rule`).** After the subroutine body is evaluated, a synthetic `RuleMatch` carrying the name message is **prepended** to the returned matches (document order: name line first). It uses the use-site offset and a dummy value, and does **NOT** advance the previous-match anchor (`terminal_anchor` still comes solely from the body's evaluation) -- the name line reads no bytes.

**Spacing model (verified against file-5.41).** The name message attaches with no leading separator: `0 name sub SUBMSG` renders `ParentSUBMSG`, not `Parent SUBMSG`. This is reproduced by ensuring the emitted message begins with the `\b` no-separator marker that `concatenate_messages` already strips (S14.1): if the name message does not already start with a marker (`\u{0008}` or literal `\b`), `evaluate_use_rule` prepends a literal `\b`. A message that ALREADY starts with one (Mach-O's `\b [`) is left as-is so it is not double-marked (`\b [` -> strip -> `[` -> `Parent [`). The `use` rule's own message is never emitted (rmagic already treated a `use` site as producing no surface `RuleMatch`), matching `file` where `0 use foo USEMSG` drops `USEMSG`.

**Stop-at-first-match interaction.** The synthetic name-message match is message-bearing, so `has_message_bearing_match` (S13.2) now counts it -- a `use` site whose subroutine's only output was the name line can now satisfy `stop_at_first_match`. This matches `file` (which would also emit and stop there). A before/after differential over the system DB confirmed the change is monotonic: the only observable diffs were 278 Mach-O universal binaries improving from malformed `x86_64] arm64e]` to balanced `[ x86_64] [ arm64e]`, with zero non-mach-o divergences.

**Scope / Mach-O status (both gaps now CLOSED -- #378).** This fix balanced the brackets; the two gaps it left are now fixed. (a) The extra space in `[ x86_64` came from the parser dropping a `use` site's whole trailing message, including a lone `\b`. `parse_name_or_use_meta` now preserves a marker that is alone on its line (every `use` site in the system DB carrying trailing text carries exactly `\b` -- 15 of them, verified) and `evaluate_use_rule` applies it to the subroutine's first message-bearing output. (b) The missing inner `:Mach-O 64-bit executable x86_64` was the indirect pointer-site base-offset bug -- see S3.10. `/usr/bin/hdiutil` now matches `file -b` byte for byte. The bare `(N)` and `indirect/r` parse gaps listed here previously are NOT mach-o rules at all: `indirect/r` is `jpeg:44`/`jpeg:86` and bare `(56)` is `games:74`; `cafebabe` contains exactly one `indirect` rule. Both are now accepted -- bare `(N)` defaults to a host-order long and `indirect/r` is consumed (see S3.10) -- so neither is a gap any longer. Regression tests: `test_use_emits_name_line_message_with_no_separator` and `test_use_nested_name_message_balances_brackets` (`tests/meta_types_integration.rs`), `test_extract_captures_name_line_message` (`parser/name_table.rs`), and the `name`/`use` message split in `test_parse_magic_rule_meta_name_use_reject_malformed_identifiers` (`parser/grammar/tests/meta_types.rs`).

### 14.5 An `indirect` Re-entry Continues Its Rule's Message (No Separator)

When a `MetaType::Indirect` rule carries its own message, the re-entered classification is appended with **no separating space** -- it continues that message rather than starting a new fragment. Mach-O's `>(8.L) indirect x \b:` renders `:Mach-O 64-bit executable x86_64`, not `: Mach-O ...`. The corroborating evidence is `archive`'s `>>>>>&-3 indirect x` rule, whose message is `\b, contains` followed by a **trailing space** -- that space is in the magic file precisely because nothing is inserted after it.

The suppression is unconditional -- it does not depend on the indirect rule carrying its own message. Two independent cases agree: mach-o's `\b:` renders `:Mach-O ...`, and jpeg's message-less `>>>10 indirect/r x` must render `[TIFF ...` directly after its sibling's `[`. An earlier revision gated this on the rule being message-bearing, which left jpeg's Exif bracket as `[ TIFF`. Implemented via `engine::output::attach_no_separator_to_first`, which marks the first match that actually renders text so a leading message-less match cannot swallow the marker. The same helper serves the `use`-site `\b` in S14.4.

**Known acceptable divergence (ADR-0002):** rmagic can out-resolve GNU `file` here. `/usr/lib/dyld`'s `arch[1].offset` is 1,064,960 -- past `file`'s ~1 MB `HOWMANY` read buffer -- so `file` prints a bare `[arm64e]` while rmagic mmaps the whole file and prints `[arm64e:Mach-O 64-bit dynamic linker arm64e]`. Verified against the fat header: that Mach-O header really is at that offset. Being more accurate than `file` is acceptable under `docs/adr/0002-accuracy-divergence.md`; do not "fix" it by discarding the correct result. The bar is *accurate*, not *more detailed* -- output the file does not support is a defect.

### 14.6 A `&MASK` With an Explicit Operator Compares in the Type's Width

`beshort&0xFFE0 =0xFFE0` parses to a `ValueTransform { BitAnd }` plus `Operator::Equal` (the fused `Operator::BitwiseAndMask` is only used for the operator-less `&MASK VALUE` form). The masked read and the literal must then share one representation: `coerce_value_to_type` narrows the literal `0xFFE0` to `i16` `-32`, so the transform result must be narrowed too -- otherwise the mask yields `i64` `65504` and the comparison fails despite identical bit patterns. `narrow_transformed_to_type_width` (`evaluator/types/mod.rs`) does that for signed `Byte`/`Short`/`Long`; libmagic compares in the type's machine word throughout.

This was silent and load-bearing: jpeg's generic APPn rule is `>0 beshort&0xFFE0 =0xFFE0`, so it never matched and `jpeg_segment` never advanced past the first segment. The implicit form (`beshort&0xFFE0 0xFFE0`) took the fused-operator path and did match, which is why the bug looked like a jpeg-specific problem rather than a general one.

### 14.7 A `use` Chain Deeper Than the Recursion Limit Degrades

Exceeding `max_recursion_depth` inside a `use` chain stops the descent and keeps whatever the shallower levels matched, rather than failing the whole file. Real magic walks a chain one `use` per element -- jpeg's `jpeg_segment` needs a level per JPEG segment, and a 10-segment JPEG plus nesting exceeds the default 20 -- so aborting would discard a legitimate partial result. The guard still prevents stack overflow; only its reporting changed. `RecursionLimitExceeded` joins `BufferOverrun` and `InvalidOffset` in the `Use` dispatch's graceful-skip arm (`evaluator/engine/mod.rs`).

### 14.8 Rendered Descriptions Are Bounded, Newline-Gated on a Narrow Idiom, and Terminal-Escaped Outside JSON

Three related bounds on rendered output, all in the description/formatting path (issue #498):

**(a) `MAX_DESCRIPTION_FIELD_LEN = 127` bounds every `%s` field read.** Defined in `src/output/format.rs`, it comes from GNU `file`'s `MAXstring - 1` (`src/file.h`) -- the last usable byte of the `MAXstring`-sized buffer `file` renders a `%s` field into. The value is **version-dependent**: `MAXstring` was 96 (95 usable) through `file` 5.38 and became 128 (127 usable) at 5.39, so 127 only holds for 5.39 and later. This is why the oracle test asserting this bound fails loudly against an older `file` rather than silently skipping. The bound is applied per read (`any_value_string_bound`, `string_ordering_display_value`, `bound_description_field`/`bound_bytes`), not accumulated across a rule chain, so a parent rule's `%s` and a child rule's `%s` each get their own 127-byte budget.

**(b) The newline-stop gate is narrower than "any ordering comparison."** `newline_stop_gate` (`src/evaluator/engine/value_eval.rs`) fires only for an any-value rule (`x`), or for an ordering-compared rule (`<`/`>`/`<=`/`>=`) whose comparison pattern's **first byte** is a null. Equality never gates. This mirrors upstream's `*m->value.s == '\0'` check, which is a first-byte test, not an empty-pattern test -- the intuitive-but-wrong reading. Measured against `file-5.41`: the 4-byte pattern `>\0ABC` gates (first byte is null) and the 1-byte pattern `>0` does not (`'0'` is `0x30`, not null).

**(c) `escape_terminal_control_bytes` (`src/cli/symlink.rs`) now also gates the ordinary magic-description text-output path.** S17.3 documents the escaped character set (C0 + DEL, C1, bidi/format overrides) and the TTY gate for symlink targets; that same function is now shared by `output_result`'s text arm (`src/main.rs`) and `output_description_bytes` (`src/cli/mod.rs`), so an attacker-controlled description rendered by an ordinary magic match is escaped under the same TTY-gated contract symlink targets already had. **Open gap:** the JSON output path does not escape -- `output_result`'s JSON arm serializes rule-derived text (descriptions, match messages) with no terminal-escaping pass. This is a real exposure a reviewer flagged and is deliberately left unfixed here pending a decision, not a claim that JSON output is safe.

Regression tests: `tests/description_bound_oracle_tests.rs` (the version-dependent 127-byte bound) and `tests/description_escaping_tests.rs` (the escaping contract).
...
## 16. `use \^name` Endian-Flip (issue #236)

### 16.1 `\^` Toggles a Threaded Flip State; It Does Not Live on a Struct Field

The magic(5) `\^` prefix on a `use` target (`>(4.L) use \^tiff_ifd`, images:314) flips the little/big endianness of every endian-bearing read inside the invoked subroutine. rmagic implements this exactly as libmagic's `softmagic.c` does:

- **`cvt_flip` equivalent** is `evaluator::types::flip_type_endian` -- it swaps `Endianness::Little` \<-> `Endianness::Big` for `Short`/`Long`/`Quad`/`Float`/`Double`/`Date`/`QDate` and returns everything else unchanged. `Endianness::Native` is left alone (libmagic's `cvt_flip` has no `FILE_SHORT`/`FILE_LONG` case), and `String16` is deliberately NOT flipped (also absent from `cvt_flip`). `signed`/`utc` attributes are preserved.
- **The flip is evaluation state, not AST shape.** libmagic threads an `int flip` parameter through `mget`/`match`; rmagic stores it on `EvaluationContext::flip_endian` and the engine's `SubroutineScope` guard saves/sets/restores it around the subroutine call (alongside `last_match_end` and `base_offset`). A `use \^name` site **toggles** (`saved_flip ^ use_flip`), it does not set -- so a `\^use` nested inside an already-flipped subroutine un-flips. The toggle **propagates into nested plain `use` calls** automatically because the nested `evaluate_use_rule` reads the now-current context flip as its own base. This is load-bearing for TIFF: `\^tiff_ifd` reads `direntries` big-endian AND its nested `>2 use tiff_entry` inherits the flip so the width/height/compression tag walk also reads big-endian. Regression: `nested_flip_use_toggles_back_to_unflipped` and the two TIFF-shape tests in `tests/endian_flip_use_tests.rs`.
- **Applied at read time only.** `evaluate_value_rule` computes a flipped `Cow<TypeKind>` for the `read_typed_value_with_pattern` call when `flip_endian` is set; `bit_width()`, `coerce_value_to_type` (the literal's numeric value is endian-invariant), the relative-offset `bytes_consumed` advance, and the string-ordering display read all keep `rule.typ` (endian-invariant for their purposes). The common unflipped path is a cheap `Cow::Borrowed` no-op.

### 16.2 `MetaType::Use` Is a Struct Variant Carrying `flip_endian` (S2.11 impact)

`MetaType::Use(String)` became `MetaType::Use { name: String, flip_endian: bool }` (issue #236). This is the standard `MetaType` exhaustive-match change (S2.11): the variant definition + serde/debug/bit_width test fixtures in `ast.rs`, the parse tag in `grammar/mod.rs::parse_name_or_use_meta` (which now records `\^` instead of `warn!`-and-discarding it), `codegen.rs::serialize_type_kind`, `strength.rs`, `property_tests.rs::arb_type_kind`, and every `MetaType::Use(name)` match site (engine dispatch, name_table lookup, test helpers) were updated. Plain `use name` parses to `flip_endian: false`.

### 16.3 Scope Boundary: Indirect Offsets Inside a Flipped Subroutine Are NOT Flipped

libmagic also applies `cvt_flip` to indirect-offset pointer-type reads inside a flipped subroutine. rmagic's implementation flips only the **value read**, not the indirect-offset pointer read. This is sufficient for the TIFF oracle (tiff_ifd/tiff_entry use only absolute offsets `>0`/`>2`/`>>8`; the `(4.L)`/`(4.l)` IFD pointer is resolved at the use SITE, before the flip toggles, so it keeps its declared `.L`/`.l` endianness correctly). A subroutine that uses an indirect offset like `(N.s)` *inside* a `\^`-flipped body would not have that pointer read flipped -- no system-DB rule the differential surfaced needs it, and the Mach-O universal per-arch inner classification does **not** need it -- `mach-o-be`/`mach-o-cpu` use only direct `byte`/`belong` offsets and a nested `use`, no indirect offset appears in either, and that path is closed by the S3.10 fix. Indirect-offset flipping stays tracked under #236 on its own merits. Do not assume indirect-offset flip works until it is implemented and tested.
