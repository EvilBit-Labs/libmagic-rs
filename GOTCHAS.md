...
- Every other escaped character — relation characters (`> < & ^ = !`), a literal backslash, an escaped `.`, an escaped space, an escaped literal tab character, and any otherwise-unrecognized escape such as `\^` — falls through getstr's `default:` case: **the backslash is dropped and the character is kept literally** (`\^` -> `^`, a regex anchor). This is the opposite of `parse_bare_string_value` (used for non-regex string-family barewords), which **keeps** the backslash on an unknown escape — regex must not reuse that path unchanged. The bullet list above is a summary of the notable named escapes, not an exhaustive taxonomy of this fallthrough bucket -- anything not named-control/octal/hex lands here, including plain punctuation that happens to be escaped for readability in a magic file.
...
**Known, tracked-but-out-of-scope divergence found while validating this fallback.** `src/evaluator/operators/bitwise.rs::apply_bitwise_and` (the `Operator::BitwiseAnd`, i.e. bare `&VALUE` relational test, as opposed to the `type&MASK` attached-mask form which uses `BitwiseAndMask`/`apply_bitwise_and_mask`) implements `(value & operand) != 0` ("any masked bit set"). Empirical A/B testing against the real `file` binary on `/usr/share/file/magic/ibm6000`'s `4 belong &0x0feeddb0` entry (a bare mask with no explicit comparison -- magic(5) shorthand for `belong &0x0feeddb0 =0x0feeddb0`) proves the correct libmagic semantics is `(value & operand) == operand` ("all masked bits set"): an all-zero masked value and a partial-bit-overlap (nonzero, but not equal to the full mask) value both correctly produce "data" from real `file`, but this crate's current implementation spuriously matches the partial-overlap case (and, because the mask has ~17 bits set, this false-positive fires for the overwhelming majority of random 32-bit values at that offset -- nearly any binary blob evaluated against the full system magic DB used to trigger a bogus "AIX core file" classification). This is a genuine, pre-existing operator-semantics bug independent of the message-less-shadowing and text/data-fallback fix in this section; fixing it is a corpus-wide-impact change to a heavily-tested, documented core operator (`test_apply_bitwise_and_symmetry`'s commutativity assumption and the existing bit-pattern test table both encode the current, wrong semantics and would need updating) and was deliberately scoped out of this fix rather than bundled in under-validated. Tracked for a dedicated follow-up.
