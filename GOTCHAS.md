...
**Known gap (still open):** the resolved XML version detail renders as `XML 1 document text` where GNU `file` prints `XML 1.0 document text` -- the sgml rule uses a `%.3s` precision specifier (`>15 string/t >\0 %.3s document text`) and `format_magic_message` does not yet honor `%s` precision. This is a display-detail bug in `src/output/format.rs`, orthogonal to the detection fix above (detection is now correct); tracked for a follow-up.
