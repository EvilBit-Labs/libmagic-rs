...
- `string` type modifier flag *parser* accepts `/c /C /w /W /b /t /T /f` but the evaluator still uses byte-exact comparison; semantics are not yet honored (tracked in issue #234). `/B` is **not** a `string` flag in libmagic — it is exclusively the `pstring` 1-byte length-width letter — so `string/B` is rejected at parse time. Note: an earlier draft of this file incorrectly listed `/T` as "force-binary"; per libmagic `src/file.h`, `/T` is `STRING_TRIM` (leading/trailing whitespace trim) and `/b` is `STRING_BINTEST` (force-binary). Bare `/f` is `STRING_FULL_WORD` (post-match word-boundary check).
